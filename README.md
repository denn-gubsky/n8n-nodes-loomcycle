...
- **LoomCycle User** — `List` / `Create` / `Update` / `Delete` / `List Tokens` / `Revoke Token` — tenant-owned users and their delegated bearer tokens (RFC BX P2; requires ≥ v1.50). The tenant is always server-derived, so no operation takes one. **Minting is intentionally absent** — the substrate returns the bearer plaintext once, and it must not land in execution data; mint via the CLI / Web UI. Note `status: disabled` also revokes the user's tokens server-side, and that `Delete` removes the identity row while leaving owned data intact (that is Erasure's job).
- **LoomCycle Usage** — `Usage Report` / `List Limits` / `Set Limit` / `Delete Limit` / `Get Config` — token + cost attribution (RFC AV; ≥ v1.10) and per-scope token budgets (RFC AW; ≥ v1.11). Group a report by `source` to see **which key actually paid** — operator vs tenant. `Set Limit` is a **full-row upsert**: a tier left at 0 is cleared to unlimited, not left alone. Budget writes stay operator-only even for a tenant member.
...
| Per-scope token budgets (RFC AW) — Set / Delete Limit | **v1.11** | Usage → List / Set / Delete Limit |
